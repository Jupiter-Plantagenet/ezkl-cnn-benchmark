#!/usr/bin/env python
"""
GPU 2: Composite Models Part 2
Runs: CNN_Mixed (2), CNN_Strided (2) = 4 experiments
"""
...
TARGET_ARCHS = ['CNN_Mixed', 'CNN_Strided']


class CompositeGPU2Runner(ExperimentRunner):
    """Runner for composite models on GPU 2"""
    
    def __init__(self):
        super().__init__()
        # Override work directory to avoid conflicts
        self.work_dir = Path('temp_gpu2_composite')
        self.work_dir.mkdir(exist_ok=True)
        
    def run_composite_experiments(self, skip_training=True):
        """Run CNN_Mixed and CNN_Strided experiments"""
        from src.models import create_composite_model
        from src.ezkl_utils import benchmark_model
        
        print(f"\n{'#'*70}")
        print(f"# GPU 2: COMPOSITE MODELS PART 2")
        print(f"# Target: CNN_Mixed (2), CNN_Strided (2)")
        ...
        for arch_name in TARGET_ARCHS:
            ...
            for tolerance in TOLERANCE_VALUES:
                ...
                print(f"\n[GPU2-{exp_count}/4] Running: {arch_name} (tolerance={tolerance})\n")
        ...
        print(f"\n{'='*70}")
        print(f"GPU 2 Composite Summary:")
        ...
        self._backup_progress("gpu2_composite_complete")


if __name__ == '__main__':
    print("="*70)
    print("GPU 2: COMPOSITE MODELS PART 2")
    print("="*70)
    print("\nGPU Assignment: CUDA_VISIBLE_DEVICES=2")
    print("Work Directory: temp_gpu2_composite/")
    print("Log File: experiment_run_composite_gpu2.log")
    print("\nExperiments to run:")
    print("  - CNN_Mixed (tol=0.5, tol=2.0)")
    print("  - CNN_Strided (tol=0.5, tol=2.0)")
    print("="*70 + "\n")
    
    runner = CompositeGPU2Runner()
    ...
    print("\n" + "="*70)
    print("GPU 2 COMPOSITE PART 2 COMPLETE")
    print("="*70)
